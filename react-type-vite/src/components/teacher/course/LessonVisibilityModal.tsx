...
import { getLessonVisibility, updateLessonVisibility } from "@/services/api/teacher/contentVisibilityApi"
...
interface LessonVisibilityModalProps {
  isOpen: boolean
  onClose: () => void
  lessonId: number
  lessonTitle: string
...
const LessonVisibilityModal: React.FC<LessonVisibilityModalProps> = ({
  isOpen,
  onClose,
  lessonId,
  lessonTitle,
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const visibilityData = await getLessonVisibility(courseId, lessonId)
      ...
      console.error("Error loading lesson visibility data:", err)
  ...
  const handleSave = async () => {
    ...
    try {
      await updateLessonVisibility(lessonId, selectedClassIds)
      ...
      console.error("Error updating lesson visibility:", err)
    ...
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl">
      <div className="flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b">
          <div>
            <h2 className="text-2xl font-bold">Quản Lý Hiển Thị Bài Học</h2>
            ...
            <>
              <div className="flex items-center justify-between bg-blue-50 p-3 rounded-lg">
                <p className="text-sm text-gray-700">
                  Chọn các lớp học được xem bài học này. Nếu không chọn lớp nào, mặc định tất cả lớp đều được xem.
                </p>
              </div>
...
export default LessonVisibilityModal