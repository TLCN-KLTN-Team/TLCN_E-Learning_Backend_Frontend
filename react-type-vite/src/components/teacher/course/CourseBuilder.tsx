...
const CourseBuilder: React.FC<CourseBuilderProps> = ({ courseId, sections, onSectionsChange, onBack }) => {
  ...
  const handleAddSection = (sectionData: Omit<SectionRequest, 'id'>) => {
    const newSection: SectionRequest = {
      ...sectionData,
      id: Date.now(), // Mock ID
      courseId: parseInt(courseId.replace('mock_id_', '')), // Mock courseId
      lessons: [],
      quizzes: [],
    };
    onSectionsChange(produce(sections, draft => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between mt-8">
        <Button variant="outline" onClick={onBack}>Back to Course Info</Button>
