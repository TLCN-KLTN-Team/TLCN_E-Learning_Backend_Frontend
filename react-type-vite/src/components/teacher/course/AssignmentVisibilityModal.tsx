...
import { getAssignmentVisibility, updateAssignmentVisibility } from "@/services/api/teacher/contentVisibilityApi"
...
interface AssignmentVisibilityModalProps {
  isOpen: boolean
  onClose: () => void
  assignmentId: number
...
const AssignmentVisibilityModal: React.FC<AssignmentVisibilityModalProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const visibilityData = await getAssignmentVisibility(courseId, assignmentId)
      ...
      console.error("Error loading assignment visibility data:", err)
  ...
  const handleSave = async () => {
    ...
    try {
      await updateAssignmentVisibility(assignmentId, selectedClassIds)
      ...
      console.error("Error updating assignment visibility:", err)
    ...
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl">
      <div className="flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b">
          <div>
            <h2 className="text-2xl font-bold">Quản Lý Hiển Thị Bài Tập</h2>
            ...
            <>
              <div className="flex items-center justify-between bg-orange-50 p-3 rounded-lg">
                <p className="text-sm text-gray-700">
                  Chọn các lớp học được xem bài tập này. Nếu không chọn lớp nào, mặc định tất cả lớp đều được xem.
                </p>
              </div>
...
export default AssignmentVisibilityModal