...
const ImportStudentsModal: React.FC<ImportStudentsModalProps> = ({
  ...
  const loadDepartments = async () => {
    try {
      ...
      const mappedDepts = deptList.map((dept: any) => ({
        id: dept.id || dept.departmentId,
        name: dept.name || dept.departmentName,
      }));
      setDepartments(mappedDepts);
      // Update the DEPARTMENT_MAP with real data
      updateDepartmentMap(deptList);
    } catch (error) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const processedData: ImportRow[] = dataRows
        .filter((row) => row.some((cell) => cell.trim() !== ""))
        .map((row, idx) => {
          const validation = validateStudentRow(row, headers, idx + 2);
          return {
            rowIndex: idx + 2,
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await studentApi.bulkImportStudents(
        educationalUnitId,
        studentRequests
      );
      ...
      toast.error(`Import thất bại: ${error.message || "Unknown error"}`);
      setStep("preview");
    } finally {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-green-50 to-emerald-50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">Import Sinh Viên</h2>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {step === "upload" && (
            <div className="space-y-6">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h3 className="font-semibold text-green-900 mb-2">Hướng dẫn Import</h3>
                <ul className="text-sm text-green-800 space-y-1 list-disc list-inside">
                  <li>Tải template XLSX bên dưới</li>
                  <li>Điền thông tin sinh viên vào file XLSX</li>
                  <li>Upload file XLSX và xem trước dữ liệu</li>
                  <li>Xác nhận để import toàn bộ sinh viên</li>
                </ul>
              </div>
              ...
              <div className="space-y-3">
                <Button
                  onClick={() => {
                    ...
                    downloadStudentTemplateXLSX(departments);
                  }}
                ...
                <label 
                  htmlFor="file-input"
                  className="block border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-green-400 transition-colors cursor-pointer"
                  role="button"
                  tabIndex={0}
                ...
                >
                  ...
                  <p className="text-gray-600 font-medium">
                    Kéo thả file XLSX tại đây hoặc click để chọn
                  </p>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-green-200">
                          ...
                          <th className="text-left py-2 px-3 font-semibold text-green-900">Mã SV</th>
                        </tr>
                      </thead>
                      <tbody>
                        {importData
                          ...
                          .map((row, idx) => (
                            <tr key={idx} className="border-b border-green-100 hover:bg-green-100">
                              ...
                              <td className="py-2 px-3">{row.data?.studentId}</td>
                            </tr>
                          ))}
                      </tbody>
                    </table>
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-600 font-semibold">Đang import {validRowsCount} sinh viên...</p>
            </div>
          )}
