...
const EnrollStudentsToClassModal: React.FC<EnrollStudentsToClassModalProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [availableResponse, enrolledResponse] = await Promise.all([
        expertClassApi.getAvailableStudentsForClass(educationalUnitId, courseClass.id),
        expertClassApi.getStudentsInClass(educationalUnitId, courseClass.id)
      ]);
  ...
  const handleEnrollStudents = async () => {
    ...
    try {
      ...
      await expertClassApi.enrollStudentsToClass(educationalUnitId, courseClass.id, studentIdsToEnroll);
  ...
  const handleUnenrollStudent = async (student: StudentResponse) => {
    ...
    if (student.submittedAssignments > 0 || student.completedQuizzes > 0 || student.viewedLessons > 0) {
      toast.error("Không thể loại bỏ sinh viên đã có tiến độ học tập (đã nộp bài tập, làm kiểm tra hoặc xem bài giảng)");
      return;
    }
  ...
  const confirmUnenrollStudent = async () => {
    ...
    try {
      setIsLoading(true);
      await expertClassApi.unenrollStudentFromClass(educationalUnitId, courseClass.id, studentToRemove.studentId);
