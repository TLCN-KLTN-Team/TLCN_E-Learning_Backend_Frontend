...
import { X, User, UserCheck, UserX, ChevronDown, Search, GraduationCap, AlertTriangle } from "lucide-react";
...
import * as teacherApi from "@/services/api/expert/expertTeacherApi";
...
const AssignTeacherModal: React.FC<AssignTeacherModalProps> = ({
  ...
  const handleRemoveTeacherClick = async () => {
    if (!course) return;
    ...
    try {
      ...
      const classesResponse = await expertClassApi.getClassesByCourse(educationalUnitId, course.id, 0, 100);
      const hasStudents = classesResponse?.content?.some(cls => (cls.currentStudents || 0) > 0);

      if (hasStudents) {
        toast.error('Không thể loại bỏ giảng viên vì đã có sinh viên trong các lớp học của môn này');
        return;
      }
      ...
      toast.error('Có lỗi xảy ra khi kiểm tra dữ liệu lớp học');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmRemoveTeacher = async () => {
    ...
    try {
      setIsLoading(true);
      await expertCourseApi.removeTeacherFromCourse(educationalUnitId, course.id);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                            ...
                            {filteredTeachers.length === 0 ? (
                              ...
                              filteredTeachers.map((teacher) => (
                                <button
                                  ...
                                  className={`w-full p-3 text-left hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0 ${selectedTeacher === teacher.teacherId ? 'bg-blue-50' : ''
                                    } `}
                                >
                                  <div className="flex items-center space-x-3">
                                    <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold text-xs">
      ...
      <AlertDialog open={showRemoveAlert} onOpenChange={(open) => {
        if (!open && !isLoading) {
          setShowRemoveAlert(false);
        }
      }}>
        ...
        <AlertDialogContent className="max-w-md bg-white rounded-xl shadow-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-center text-xl">
              Xác nhận loại bỏ giảng viên
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="text-center">
                <div className="bg-gray-50 rounded-lg p-4 mt-4">
                  <p className="text-sm text-gray-600 mb-3">
                    Bạn có chắc chắn muốn loại bỏ giảng viên này khỏi môn học?
                  </p>
                  {course.teacher && (
                    <div className="flex items-center justify-center space-x-3 p-3 bg-white rounded-lg border border-gray-200">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs text-gray-500">
                          Mã GV: {course.teacher.teacherId}
                        </div>
                      </div>
                    </div>
                  )}
                  <p className="text-sm text-red-500 mt-3 font-medium">
                    Hành động này sẽ xóa quyền quản lý điểm và lớp học của giảng viên đối với môn này.
                  </p>
                </div>
              </div>
            </AlertDialogDescription>
